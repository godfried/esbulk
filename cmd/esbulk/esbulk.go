...
var (
	...
	disableRefreshInterval = flag.Bool("norefresh", false, "disable refresh interval during indexing")
)
...
func main() {
	...
	for i, _ := range options.Servers {
		...
		indexSettings := doc[options.Index].(map[string]interface{})["settings"].(map[string]interface{})["index"].(map[string]interface{})
		numberOfReplicas := indexSettings["number_of_replicas"]
		refreshInterval := indexSettings["refresh_interval"]
		if *verbose {
